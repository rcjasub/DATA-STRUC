...
public:
    ...
    void reverseBetween(int L, int R)
    {
        ...
        for (int i = 0; i < L - 1; i++)
            pre = pre->next; // pre points to the node before the L node
    ...
    Node *OddAndEvenList()
    {
        ...
        std::queue<Node *> oddQueue, evenQueue;
        Node *temp = head;
        ...
        Node *dummy = new Node(0); // create a fake starter node
        Node *tail = dummy;

        // Loop through both queues until both are empty
        // grab odd first then even, til both are empty
        while (!oddQueue.empty() || !evenQueue.empty())
        {
            // 1st
            if (!oddQueue.empty())
            {
                tail->next = oddQueue.front();
                oddQueue.pop(); // clear the front node from the queue
                tail = tail->next;
            }
        ...
        delete dummy;    // Free the dummy node
        return nextNode; // Return the new head of the rearranged list
    }

    // check if the linked list has a cycle(loop)
    bool hasCycle()
    {
        if (head == nullptr)
            return false;
        ...
        while (fast != nullptr && fast->next != nullptr)
        {
            ...
            // slow is on a node, fast skips slow but if it points to slow, then there is a cycle
            if (slow == fast)
            {
                return true;
            }
